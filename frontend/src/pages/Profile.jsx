...
const Profile = () => {
  // Grab our new updateProfilePic function and API_URL from context
  const { user, logout, updateProfilePic, API_URL } = useContext(AuthContext);
  const navigate = useNavigate();
  ...
  const handleFileChange = async (e) => {
    ...
    // Pack the file to send to FastAPI
    ...
    try {
      ...
      // Construct the full image URL and update global context
      const fullImageUrl = API_URL + response.data.url;
          ...
          <div className="h-32 bg-gradient-to-r from-[#0039A6] to-[#002875] relative">
            ...
            <div className="absolute -bottom-12 left-8 group">
              <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-lg border-4 border-white overflow-hidden relative transition-transform group-hover:scale-105">
                {isUploading ? (
                  <Loader2 className="w-8 h-8 text-[#0039A6] animate-spin" />
                ) : user.profilePic ? ( // <-- READING DIRECTLY FROM CONTEXT NOW
