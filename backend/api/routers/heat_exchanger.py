...
# from schemas.exp_schema import BatchRequest
# from engine.physics import calculate_analytical_th_out
# from engine.ml_models import predict_ml_heat_exchanger
# from api.dependencies import get_db
...
#     physics_out = calculate_analytical_th_out(th_in, tc_in, m_h, m_c)
#     ai_out = predict_ml_heat_exchanger(th_in, tc_in, m_h, m_c)
...
from schemas.exp_schema import BatchRequest
from engine.physics import calculate_analytical_th_out
from engine.ml_models import predict_ml_heat_exchanger # Updated import name

router = APIRouter(tags=["Heat Exchanger"])

def process_single_row(row_data: dict) -> dict:
    """Helper function to run the math and ML models for a single row."""
    th_in = float(row_data.get('th_in', 0))
    tc_in = float(row_data.get('tc_in', 0))
    m_h = float(row_data.get('m_h', 0))
    m_c = float(row_data.get('m_c', 0))
    ...
    # 1. Physics Engine
    th_out_phys = calculate_analytical_th_out(th_in, tc_in, m_h, m_c)
    # Basic Energy Balance for TC_out: Q_h = Q_c => m_h*Cp*(th_in - th_out) = m_c*Cp*(tc_out - tc_in)
    # tc_out = tc_in + (m_h * (th_in - th_out)) / m_c
    tc_out_phys = tc_in + (m_h * (th_in - th_out_phys)) / m_c if m_c > 0 else tc_in

    # 2. ML Engine (Now returns dict: {"th_out": ..., "tc_out": ...})
    ai_out = predict_ml_heat_exchanger(th_in, tc_in, m_h, m_c)
    
    # 3. Calculate Error (Average percentage error between ML and Physics for both)
    th_err = abs((ai_out["th_out"] - th_out_phys) / th_out_phys) * 100 if th_out_phys != 0 else 0
    tc_err = abs((ai_out["tc_out"] - tc_out_phys) / tc_out_phys) * 100 if tc_out_phys != 0 else 0
    avg_error = (th_err + tc_err) / 2

    return {
        "input": {"th_in": th_in, "tc_in": tc_in, "m_h": m_h, "m_c": m_c},
        "ml_out": ai_out, # Contains both th_out and tc_out
        "analytical_out": {"th_out": round(th_out_phys, 2), "tc_out": round(tc_out_phys, 2)},
...
async def run_file_simulation(file: UploadFile = File(...)):
    ...
    # Map input keys to expected schema
    df = df.rename(columns={'th_in': 'th_in', 'tc_in': 'tc_in', 'm_h': 'm_h', 'm_c': 'm_c'})
    results = [process_single_row(row.to_dict()) for _, row in df.iterrows()]
    return {"results": results}